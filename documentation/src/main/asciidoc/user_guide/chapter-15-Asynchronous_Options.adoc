...
[[sid-8093935_AsynchronousOptions-Introduction]]
...
On the contrary, if your application only ever makes one modification per key/value pair and there's no happens-before relationship between them, then async marshalling is a very valid optimization that can increase performance of your application without data consistency risks.
 ...
 If you have async marshalling turned on and see exceptions related to java.util.concurrent.RejectedExecutionException , as explained in the <<sid-8093846,technical faq page>> , you should also consider switching off async marshalling. 
...
[[sid-8093935_AsynchronousOptions-ReplicationQueue]]
...
[[sid-8093935_AsynchronousOptions-AsynchronousAPI]]
...
[[sid-8093935_AsynchronousOptions-ReturnValues]]
